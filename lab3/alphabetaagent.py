...
class AlphaBetaAgent(object):
    ...
    def decide(self, connect4):
        if connect4.who_moves != self.my_token:
            score, move = self.alphabeta(connect4, 0, 4, float('-inf'), float('inf'))
        else:
            score, move = self.alphabeta(connect4, 1, 4, float('-inf'), float('inf'))
        return move
    ...
    def alphabeta(self, connect4, x, d, alpha, beta):
        ...
        if x == 1:
            ...
            for move in connect4.possible_drops():
                ...
                score, _ = self.alphabeta(connect4_copy, 0, d - 1, alpha, beta)
                if score > best_score:
                    best_score = score
                    best_move = move
            ...
            for move in connect4.possible_drops():
                connect4_copy = copy.deepcopy(connect4)
                connect4_copy.drop_token(move)
                score, _ = self.alphabeta(connect4_copy, 1, d - 1, alpha, beta)
