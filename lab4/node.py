...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            ...
            left_pos, left_neg = np.sum((left_split == 1)), np.sum((left_split == 0))
            right_pos, right_neg = np.sum((right_split == 1)), np.sum((right_split == 0))
            ...
            gain = 1 - (left / (left + right) * gini_left) - (right / (left + right) * gini_right)
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        # feature_subset - number of features to select
        num_of_features = X.shape[1]  # number of features to select from

        if feature_subset is not None:
            selected_features = np.random.choice(num_of_features, feature_subset, replace=False)
