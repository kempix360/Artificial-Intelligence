...
def training(model, x, y):
    ...
    for _ in range(n_steps):
        ...
        indices = torch.randint(0, x.shape[0], (minibatch_size,))  # generate random index from 0 to x.shape[0]-1
        x_batch = x[indices]
        y_batch = y[indices]
        ...
        for p in model.parameters():
            p.grad = None  # gradienty są akumulowane, więc przed następnym krokiem trzeba je zresetować (specyfika
            # pyTorch)
