...
def zad2_two_layer_net(student_id):
    ...
    model.hidden_layer.W[:, 0] = [0.2, -1]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [-0.1, 1]   # wagi neuronu h2
    model.hidden_layer.b[:] = [0, -0.8]         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [2, 2]   # wagi neuronu wyjściowego
